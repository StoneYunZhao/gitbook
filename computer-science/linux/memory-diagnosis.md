...
{% hint style="info" %}
磁盘是块设备，可以划分不同分区。分区之上可以再创建文件系统，挂载到某个目录。通常写文件时会经过文件系统，文件系统负责与磁盘交互。裸 IO 直接读写磁盘或分区，会跳过文件系统。注意裸 IO 与直接 IO 的区别，直接 IO 跳过 PageCache，也会跳过 buffer，直达块层，需要用户处理对齐问题。
{% endhint %}
